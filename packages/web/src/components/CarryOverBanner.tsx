...
export default function CarryOverBanner({ date }: Props) {
  ...
  const carryOver = useMutation({
    ...
    onSuccess: () => {
      // Only invalidate today + yesterday, not all cached daylogs
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium">
            {count} incomplete task{count !== 1 ? "s" : ""} from yesterday
          </p>
        ...
        >
          {carryOver.isPending ? "Moving..." : (
            <>
              Carry over <ArrowRight size={12} />
            </>
          )}
        </button>
        <button
